...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig("img/electron_density_1.png", dpi=200)
    plt.show(block=False)
    ...
    fig.colorbar(im, ax=axs[1], label='Ne (m$^{-3}$)')
    plt.tight_layout()
    plt.savefig("img/electron_density_2.png", dpi=200)
    plt.show(block=False)
    ...
    # Expanded ray generation: cover all combinations of theta and integration time
    theta_list = np.linspace(30, 85, 20)  # 20 angles from horizontal
    integration_times_hf_grid = np.linspace(0.08, 0.14, 20)  # 20 integration times
    ...
    all_vtec_hf = []
    all_deltat_hf = []
    integration_times_hf_list = []
    theta_per_ray = []

    for θ in theta_list:
        for T_hf in integration_times_hf_grid:
            ...
            all_vtec_hf.append(vtec)
            all_deltat_hf.append(delta_t)
            integration_times_hf_list.extend([T_hf] * len(rays_hf))
            theta_per_ray.extend([θ] * len(rays_hf))

    all_vtec_hf = np.hstack(all_vtec_hf)
    all_deltat_hf = np.hstack(all_deltat_hf)
    integration_times_hf = np.array(integration_times_hf_list)
    ...
    plt.tight_layout()
    plt.savefig("img/raypath.png", dpi=200)
    plt.show(block=False)
...
# Add a small value to all bins to fill the colormap
...
plt.pcolormesh(xedges, yedges, avg_error, shading='auto', cmap='plasma', vmin=0, vmax=np.nanmax(avg_error))
plt.xlabel("Integration Time (s)")
plt.ylabel("TEC (electrons/m²)")
plt.title("Average TEC Estimate Error for Passive (Simulated)")
plt.colorbar(label="ΔTEC Error (m⁻²)")
plt.tight_layout()
plt.savefig("img/avg_tec_error_passive.png", dpi=200)
plt.show(block=False)
...
plt.colorbar(label="ΔTEC Error (m⁻²)")
plt.tight_layout()
plt.savefig("img/avg_tec_error_passive_2.png", dpi=200)
plt.show(block=False)
...
plt.pcolormesh(xedges_r, yedges_r, avg_range_err, shading='auto', cmap='jet', vmin=0, vmax=np.nanmax(avg_range_err))
...
plt.tight_layout()
plt.savefig("img/vhf_range_error_after_hf_correction.png", dpi=200)
plt.show(block=False)
...
plt.tight_layout() 
plt.savefig("img/ang_res_passive.png", dpi=200)
plt.show(block=False)
...
plt.tight_layout()
plt.savefig("img/tec_error_passive.png", dpi=200)
plt.show(block=False)
...
plt.tight_layout()
plt.savefig("img/tec_error_coverage_passive.png", dpi=200)
plt.show(block=False)


# anotehr addition
# Overlay binned mean/std on the TEC–T heatmap
...
plt.tight_layout()
plt.savefig("img/binned_mean_tec_error_passive.png", dpi=200)
plt.show(block=False)
...
plt.tight_layout()
plt.savefig("img/inversion_art_1.png", dpi=200)
plt.show(block=False)
...
plt.title("Inversion quality vs θ coverage (ART)")
plt.tight_layout()
plt.savefig("img/inversion_art_2.png", dpi=200)
plt.show(block=False)
